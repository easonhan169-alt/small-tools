...
def load_csv(source: BinaryIO | bytes) -> "Workbook":
    """Load a CSV file into a single-sheet workbook so it can be compared like Excel."""
    from openpyxl.workbook import Workbook
...
def get_document_format(filename: str) -> str:
    """Return the normalized supported document format for an uploaded filename."""
...
def load_spreadsheet(source: BinaryIO | bytes, filename: str) -> "Workbook":
    """Load a supported Excel or CSV document as a workbook for comparison."""
    if get_document_format(filename) == "csv":
...
def create_replacement_csv_bytes(replacement_wb: "Workbook") -> bytes:
    """Create UTF-8 BOM CSV bytes from the first worksheet in a replacement workbook."""
    output = io.StringIO(newline="")
    writer = csv.writer(output)
    worksheet = replacement_wb.worksheets[0]
...
def create_replacement_file_bytes(source_wb: "Workbook", replacement_wb: "Workbook", output_format: str) -> bytes:
    """Create replacement output bytes in the requested source document format."""
    if output_format == "csv":
        return create_replacement_csv_bytes(replacement_wb)
    return create_replacement_workbook_bytes(source_wb, replacement_wb)


def get_replacement_download_metadata(original_filename: str, label: str) -> tuple[str, str, str]:
    """Return download filename, MIME type, and output format for a replacement file."""
    output_format = get_document_format(original_filename)
    if output_format == "csv":
        return f"{label}.csv", "text/csv", "csv"
    return (
        f"{label}.xlsx",
...
def render_app() -> None:
    ...
    st.caption("上传两个 Excel/CSV 文件，查看结构、数据和字符级差异，并生成一键替换后的文件。")

    supported_types = ["xlsx", "xlsm", "xltx", "xltm", "csv"]
    left_file = st.file_uploader("左侧/原始文档（Excel/CSV）", type=supported_types, key="left")
    right_file = st.file_uploader("右侧/目标文档（Excel/CSV）", type=supported_types, key="right")

    if not left_file or not right_file:
        st.info("请上传两个 Excel 或 CSV 文件开始对比。")
        return

    left_bytes = left_file.getvalue()
    right_bytes = right_file.getvalue()
    left_wb = load_spreadsheet(left_bytes, left_file.name)
    right_wb = load_spreadsheet(right_bytes, right_file.name)
    ...
    left_filename, left_mime, left_format = get_replacement_download_metadata(
        left_file.name, "left-replaced-by-right"
    )
    right_filename, right_mime, right_format = get_replacement_download_metadata(
        right_file.name, "right-replaced-by-left"
    )
    left_to_right = create_replacement_file_bytes(left_wb, right_wb, left_format)
    right_to_left = create_replacement_file_bytes(right_wb, left_wb, right_format)
