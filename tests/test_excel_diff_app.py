...
from src.excel_diff_app import (
    build_char_diff_html,
    compare_workbooks,
    create_replacement_csv_bytes,
    create_replacement_workbook_bytes,
    get_column_letter,
    get_replacement_download_metadata,
    load_spreadsheet,
)
...
def test_load_spreadsheet_supports_csv_files():
    pytest.importorskip("openpyxl")
    workbook = load_spreadsheet("名称,数量\n苹果,2\n".encode("utf-8-sig"), "items.csv")

    assert workbook.sheetnames == ["CSV"]
    assert workbook["CSV"]["A1"].value == "名称"
    assert workbook["CSV"]["B2"].value == "2"


def test_create_replacement_csv_bytes_uses_first_sheet_content_when_openpyxl_is_available():
    openpyxl = pytest.importorskip("openpyxl")
    workbook = openpyxl.Workbook()
    workbook.active.append(["名称", "数量"])
    workbook.active.append(["苹果", 2])

    output = create_replacement_csv_bytes(workbook).decode("utf-8-sig")

    assert output == "名称,数量\r\n苹果,2\r\n"


def test_replacement_download_metadata_preserves_csv_extension():
    assert get_replacement_download_metadata("left.csv", "left-replaced-by-right") == (
        "left-replaced-by-right.csv",
        "text/csv",
        "csv",
    )


def test_create_replacement_workbook_bytes_copies_styles_without_broken_style_indexes():
    openpyxl = pytest.importorskip("openpyxl")
    from openpyxl.styles import Alignment, Font, PatternFill

    source = openpyxl.Workbook()
    replacement = openpyxl.Workbook()
    cell = replacement.active["A1"]
    cell.value = "styled"
    ...
    assert output["Sheet"]["A1"].value == "styled"
    assert output["Sheet"]["A1"].font.bold is True
    assert output["Sheet"]["A1"].alignment.horizontal == "center"
